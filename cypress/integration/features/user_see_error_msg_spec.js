describe("error messages", () => {
    ...
    it("when bad status code", () => {
        cy.server();
        cy.route({url: "**//www.dictionaryapi.com/**", status : 503, response : {}}).as('search');
        cy.search("hello")
        cy.wait("@search");
        cy.get("[data-cy=msg]").contains("Something's wrong with Merriam-Webster API, try later?");
    });
    ...
    it("when time out", () => {
        cy.server();
        cy.route({url: "**//www.dictionaryapi.com/**", delay : 3000, response: {}}).as('search');
        cy.search("hello");
        cy.wait("@search");
